...
"""
Script autocontenido y final para generar el dashboard de Análisis de Riesgos
para el informe de Wasaff Consulting a Synergy Fire Protection.
- Versión 12/10: Calidad de Publicación.
"""
...
PIPE_DATA: Dict[str, Dict[str, Any]] = {
    ...
    "12":   {"OD": 323.8, "SCH": {10: 4.57, 40: 10.31}}
}
...
def crear_dashboard_riesgos(df: pd.DataFrame, output_dir: str):
    ...
    energia_vehiculo_kj = 0.5 * 1400 * (50/3.6)**2 / 1000
    energia_tubo_pequeno = df[(df['NPS (pulgadas)'] == '4') & (df['Schedule'] == 40) & (df['Altura Caída (m)'] == 1)]['Energía Impacto (kJ)'].iloc[0]
    energia_tubo_grande = df[(df['NPS (pulgadas)'] == '12') & (df['Schedule'] == 40)]['Energía Impacto (kJ)'].max()
    escenarios = {
    ...
    # Guardado
    output_path = os.path.join(output_dir, 'dashboard_analisis_riesgos.pdf')
    plt.savefig(output_path, bbox_inches='tight')
    print(f"Dashboard guardado como '{output_path}'.")
...
if __name__ == "__main__":
    ...
    # 1. Ejecutar el análisis para generar los datos
    data_rows = []
    for nps, props in PIPE_DATA.items():
        for sch, t_mm in props["SCH"].items():
            ...
            for height in HEIGHTS_M:
                data_rows.append({
                    "NPS (pulgadas)": nps, "Schedule": int(sch), "Altura Caída (m)": height,
    ...
    # 2. Asegurar que el directorio de salida exista
    output_directory = 'Figuras'
    try:
        os.makedirs(output_directory, exist_ok=True)
        print(f"Directorio '{output_directory}' listo.")
    except OSError as e:
        print(f"Error creando el directorio '{output_directory}': {e}")
        exit() # Salimos si no podemos crear el directorio

    # 3. Generar la visualización
    crear_dashboard_riesgos(df_riesgos, output_directory)
