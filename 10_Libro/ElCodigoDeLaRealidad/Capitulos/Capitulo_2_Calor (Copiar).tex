...
En el Capítulo~\ref{chap:poisson}, nuestra simulación proporcionó una instantánea, una ``fotografía'', del sistema: el perfil de temperatura de la tubería de concentrado en un estado de equilibrio a largo plazo. La conclusión fue alarmante: una parte significativa del concentrado se solidifica. Sin embargo, esta imagen estática no narra la historia completa. El mundo físico es dinámico; los sistemas evolucionan, y la ingeniería de procesos a menudo exige comprender la \textit{tasa} a la que ocurren estos cambios.

Este capítulo introduce la dimensión del \textbf{tiempo}. Transitaremos del análisis estacionario al transitorio, modelando la evolución dinámica del sistema para crear un ``vídeo'' de la física. Responderemos a la pregunta crítica planteada al final del capítulo anterior: tras una parada no planificada del flujo, \textit{¿cuál es la ventana de tiempo operativa antes de que la solidificación cause una falla catastrófica?} Para abordar esta cuestión, derivaremos la \textbf{Ecuación del Calor}, la contraparte parabólica de la Ecuación elíptica de Poisson.

\begin{center}
\begin{tabular}{|p{0.9\textwidth}|}
\hline
\textbf{CONTEXTO INGENIERIL} \\
El análisis transitorio es el corazón de la ingeniería de operaciones y la gestión de riesgos. En el contexto de nuestra tubería de concentrado, nos permite cuantificar:
\begin{itemize}
    \item \textbf{Tiempos de Respuesta Críticos:} Determinar la ventana de tiempo segura para que los operadores reinicien un sistema o para que el personal de mantenimiento realice una reparación antes de que ocurra una falla catastrófica (el tapón).
    \item \textbf{Protocolos de Operación:} Diseñar procedimientos para arranques y paradas seguras, conociendo cómo evoluciona la temperatura del sistema.
    \item \textbf{Evaluación de Contingencias:} Simular escenarios de falla (``¿qué pasa si la bomba se detiene por 3 horas?'') para desarrollar planes de mitigación robustos.
\end{itemize}
Modelar la dinámica temporal es esencial para pasar de un diseño estático a un sistema operable y seguro. \\
\hline
\end{tabular}
\end{center}

Para incorporar la dependencia temporal, debemos volver al primer principio de conservación de la energía. Sin embargo, esta vez consideraremos el término que define toda la dinámica de los procesos difusivos: la \textbf{acumulación (o disipación) de energía} en el tiempo.

\subsection{La Ecuación Fuerte Dependiente del Tiempo}
Sea un intervalo de tiempo de interés \(I = (0, t_f]\). Buscamos ahora una función de temperatura \(T(\bm{x}, t)\) definida sobre el dominio espacio-temporal \(\Omega \times I\).
